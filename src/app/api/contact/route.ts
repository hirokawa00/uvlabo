...
const contactSchema = z.object({
  name:     z.string().trim().min(1,"お名前を入力してください").max(50,"件名が大きすぎます"),
  email: z
    .string()
    .trim()
...
app.post(
  ...
  async (c) => {
    ...
    if (!resendApiKey || !toEmail) {
      console.log("【お問い合わせ受信（Resend未設定）】", { name, email, category, message })
...
https://uvlabo.com
    ...
    try {
      ...
      if (!adminRes.ok) {
        ...
        return c.json({ error: "メールの送信に失敗しました。時間をおいて再度お試しください。" }, 500)
      }
