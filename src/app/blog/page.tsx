...
export const metadata: Metadata = {
  title: "UV・紫外線ケアコラム",
  description:
    "日焼け止めの選び方・UV指数の読み方・日焼けあとのケア方法など、紫外線対策に関する情報をまとめたコラム一覧です。",
};
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main className="flex-1 max-w-2xl mx-auto w-full px-4 py-8">
        {/* ページタイトル */}
        <div className="mb-8">
          <p className="text-xs font-medium text-emerald-700 tracking-widest mb-1.5">
            COLUMN
          </p>
          ...
          <div className="flex flex-wrap gap-2">
            {(
              Object.entries(CATEGORY_CONFIG) as [
                BlogCategory,
                (typeof CATEGORY_CONFIG)[BlogCategory],
              ][]
            ).map(([key, cfg]) => {
...
function FeaturedCard({
  ...
  post: ReturnType<typeof getFeaturedPosts>[0];
}) {
  ...
  return (
    <Link href={`/blog/${post.slug}`} className="block group">
      <div className="rounded-2xl border border-border overflow-hidden hover:border-emerald-300 transition-colors">
        {/* カバーグラデーション */}
        <div
          className={`bg-linear-to-r ${post.coverColor} px-6 py-8 flex items-center justify-between`}
        >
          <span className="text-5xl">{post.coverEmoji}</span>
          <span
            className={`text-xs px-2.5 py-1 rounded-full font-medium ${cfg.color} ${cfg.textColor}`}
          >
            {cfg.emoji} {cfg.label}
          </span>
        </div>
  ...
  return (
    <Link href={`/blog/${post.slug}`} className="block group">
      <div className="flex items-center gap-4 p-4 rounded-xl border border-border hover:border-emerald-300 hover:bg-emerald-50/30 transition-all">
        <div
          className={`w-12 h-12 rounded-xl bg-linear-to-br ${post.coverColor} flex items-center justify-center text-2xl shrink-0`}
        >
          {post.coverEmoji}
        </div>
        <div className="flex-1 min-w-0">
          <span
            className={`text-[10px] px-1.5 py-0.5 rounded font-medium ${cfg.color} ${cfg.textColor} mb-1 inline-block`}
          >
            {cfg.label}
          </span>
