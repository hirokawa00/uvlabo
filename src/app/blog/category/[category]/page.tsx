  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main className="flex-1 max-w-2xl mx-auto w-full px-4 py-8">
        ...
        {/* カテゴリヘッダー */}
        <div className="mb-8">
          <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium mb-4 ${cfg.color} ${cfg.textColor}`}>
            <span>{cfg.emoji}</span>
            {cfg.label}
          </div>
        ...
        {posts.length > 0 ? (
          <div className="space-y-3">
            {posts.map((post) => (
              ...
              >
                <div className={`w-14 h-14 rounded-xl bg-linear-to-br ${post.coverColor} flex items-center justify-center text-2xl shrink-0`}>
                  {post.coverEmoji}
                </div>
