...
function buildMatchReason(product: Product, input: DiagnosisInput): string {
  ...
  if (
    input.makeupStyle === "with_makeup" &&
    product.tags.includes("下地兼用")
  ) {
  ...
  if (
    product.tags.includes("ウォータープルーフ") &&
    input.scene === "outdoor"
  ) {
...
function buildSkinAdvice(skinType: SkinType): string {
  const advice: Record<SkinType, string> = {
    ...
    combination:
      "混合肌はTゾーンと頬で使い分けがベスト。軽いテクスチャで皮脂コントロールできるタイプを選びましょう。",
    sensitive:
      "敏感肌には紫外線吸収剤不使用のノンケミカルタイプを。パッチテスト済み・アレルギーテスト済みの表記も確認してください。",
    normal:
      "普通肌はSPF値と使用感の好みで選べます。日常使いはSPF30〜50、屋外活動はSPF50+を使い分けると肌への負担を減らせます。",
  };
...
function buildSceneAdvice(scene: UseScene): string {
  const advice: Record<UseScene, string> = {
    commute:
      "通勤時は帰宅後に落としやすい石けんオフタイプがおすすめ。電車内の窓ガラスからも紫外線は透過するため、座席側の腕・手にも塗布を忘れずに。",
    outing:
      "お出かけ時は携帯しやすいスプレーやスティックタイプも活用して、2〜3時間ごとに塗り直しを。ランチ後にも一度塗り直すと安心です。",
    outdoor:
      "アウトドアでは汗・水に強いウォータープルーフタイプを選び、90分〜2時間ごとに必ず塗り直してください。帽子・サングラスと組み合わせると防御力が大幅にアップします。",
    indoor:
      "室内でも窓際の紫外線（UVA）は防ぐ必要があります。SPF15〜30程度の軽いタイプを朝1回塗るだけで十分です。",
  };
  return advice[scene];
}
...
export function runDiagnosis(input: DiagnosisInput): DiagnosisResult {
  // 全製品をスコアリング
  const scored = PRODUCTS.filter((p) => p.spf > 0) // 事後ケア製品は除外（別途表示）
    .map((p) => ({
      ...p,
