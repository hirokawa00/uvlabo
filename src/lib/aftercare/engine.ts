...
const AFTERCARE_PRODUCTS: AfterCareProduct[] = [
  ...
  },
  {
    ...
    affiliateUrl:
      "https://www.amazon.co.jp/s?k=アロエジェル+日焼け&tag=uvlab-22",
    source: "amazon",
  },
  {
    ...
    useCase: "日焼け後の保湿・鎮静に",
    affiliateUrl:
      "https://www.amazon.co.jp/s?k=無香料+化粧水+日焼け後&tag=uvlab-22",
    source: "amazon",
  },
];
...
const TONIGHT_STEPS: Record<string, StepTemplate> = {
  cool_water: {
    ...
    title: "まず流水で冷やす",
    detail:
      "患部を15〜20分間、流水（20℃前後）で冷やしてください。氷や保冷剤を直接当てるのはNGです。",
    duration: "15〜20分",
  ...
  cool_towel: {
    ...
    title: "冷やしたタオルで冷却",
    detail:
      "水で濡らして絞ったタオルを患部に当てて冷却します。15〜20分を目安に繰り返してください。",
    duration: "15〜20分",
  ...
  gentle_cleansing: {
    ...
    title: "ぬるま湯でやさしく洗顔",
    detail:
      "38℃前後のぬるま湯でやさしく洗い流します。洗顔料は使わないか、泡立てた泡のみで摩擦を最小限に。",
  ...
  aloe_gel: {
    ...
    title: "アロエジェルで鎮静",
    detail:
      "冷やしたアロエベラジェルを患部にたっぷりのせて、なじませます。こすらずパッティングで。",
  ...
  hydration: {
    ...
    title: "化粧水でたっぷり保湿",
    detail:
      "無香料・アルコールフリーの化粧水を手のひらで押さえるようにして浸透させます。コットンでのパッティングはこの時期は避けましょう。",
  ...
  moisturize: {
    ...
    title: "乳液・クリームで蓋をする",
    detail:
      "化粧水の後に保湿クリームでしっかり蓋をします。セラミド配合や無香料タイプが肌に優しいです。",
  },
  no_bath: {
    order: 6,
    title: "今夜はシャワーのみ",
    detail:
      "入浴は炎症を悪化させる可能性があるため、ぬるめのシャワーだけにとどめましょう。サウナ・岩盤浴は厳禁です。",
  ...
  rest: {
    ...
    title: "十分な睡眠をとる",
    detail:
      "皮膚の修復は睡眠中に行われます。できるだけ早めに就寝し、7〜8時間の睡眠を確保しましょう。",
  },
};
...
const DAY1_3_STEPS: Record<string, StepTemplate> = {
  uv_avoidance: {
    ...
    title: "紫外線を徹底的に避ける",
    detail:
      "日焼けした翌日からの数日間は、肌が最も紫外線の影響を受けやすい状態です。外出時はUV服・帽子・日傘で物理的に遮断し、日焼け止めも必ず使用してください。",
    productIds: [],
  },
  continue_hydration: {
    order: 2,
    title: "保湿を1日3〜4回続ける",
    detail:
      "日焼け後の肌は水分を失いやすい状態です。化粧水 → 乳液 → クリームのスキンケアを1日3〜4回行い、乾燥させないことがシミ予防の鍵です。",
    productIds: ["amazon-lotion-mild"],
  },
  vitamin_c: {
    order: 3,
    ...
    detail:
      "食事からビタミンC（レモン・キウイ・パプリカ）とビタミンE（アーモンド・アボカド）を積極的に摂取しましょう。メラニン生成の抑制を内側からサポートします。",
  },
  no_exfoliation: {
    order: 4,
    title: "スクラブ・ピーリングはNG",
    detail:
      "炎症が残っている間は、スクラブや角質ケアは絶対に避けてください。傷んだ肌をさらに傷め、色素沈着の原因になります。",
  ...
  sunscreen_even_indoor: {
    ...
    title: "室内でも日焼け止めを塗る",
    detail:
      "窓から入るUVAは普通のガラスを透過します。在宅日でもSPF15〜30の日焼け止めを朝に塗布してください。",
  },
};
...
const DAY4_PLUS_STEPS: Record<string, StepTemplate> = {
  whitening_care: {
    ...
    title: "美白ケアを開始する",
    detail:
      "炎症が落ち着いた4日目頃から、ハイドロキノンやビタミンC誘導体配合の美白アイテムを導入します。メラニンが定着する前にケアを始めることが重要です。",
  ...
  body_care: {
    ...
    title: "ボディの黒ずみケアを開始",
    detail:
      "腕・脚・デコルテの日焼けあとには、美白成分配合のボディジェルやクリームを毎日塗布します。シャワー後の肌が柔らかい状態でのケアが効果的です。",
  ...
  uv_resume: {
    ...
    title: "通常のスキンケアに戻す",
    detail:
      "1週間程度で皮膚のバリア機能が回復します。普段のスクラブや角質ケアも徐々に再開してOKです。ただし日焼け止めとUV対策は継続してください。",
  },
  dermatologist: {
    order: 4,
    title: "2週間以上改善しない場合は皮膚科へ",
    detail:
      "赤みや色素沈着が2週間以上改善しない場合は、皮膚科でのトレチノインやハイドロキノン処方を検討しましょう。",
...
function buildTonightSteps(
  ...
  if (elapsed === "within3h") {
    steps.push(
      level === "severe" ? TONIGHT_STEPS.cool_towel : TONIGHT_STEPS.cool_water,
    );
  }
...
function buildRecommendedProducts(
  ...
  if (level !== "mild") {
    products.push(
      AFTERCARE_PRODUCTS.find((p) => p.id === "kiso-hydroquinone")!,
    );
  } else {
    products.push(
      AFTERCARE_PRODUCTS.find((p) => p.id === "kiso-hydroquinone")!,
  ...
  if (areas.some((a) => bodyAreas.includes(a))) {
    products.push(
      AFTERCARE_PRODUCTS.find((p) => p.id === "siBody-mineral-shine")!,
    );
  }
