      ...
      <div className="space-y-2.5">
        {related.map((post) => {
          ...
          return (
            ...
            >
              <div
                className={`w-11 h-11 rounded-xl bg-linear-to-br ${post.coverColor} flex items-center justify-center text-xl shrink-0`}
              >
                {post.coverEmoji}
              </div>
...
export function CategoryNav({ category, currentSlug }: CategoryNavProps) {
  ...
  return (
    <div
      className={`p-4 rounded-2xl border ${cfg.color.replace("bg-", "border-").replace("100", "200")} bg-linear-to-br ${cfg.color}/30 to-transparent`}
    >
      <p
        className="text-xs font-bold tracking-widest mb-3 flex items-center gap-1.5"
        style={{ color: "inherit" }}
      >
        <span>{cfg.emoji}</span>
